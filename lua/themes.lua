...
-- Available themes
M.themes = {
  { name = "zone_green", display = "Zone Green" },
  { name = "monokai-pro", display = "Monokai Pro (Zen)" },
  { name = "gruvbox-material", display = "Gruvbox Material (Zen)" },
  { name = "tokyonight", display = "Tokyo Night (Zen)" },
}
...
M.set_theme = function(theme_name)
  ...
  elseif theme_name == "gruvbox" then
    ...
    if ok then
      ...
      overrides = {
        ...
        LineNr = { fg = "#004000", bg = "#000000" },
        CursorLineNr = { fg = "#00ff00", bg = "#0a0a0a" },
        Visual = { bg = "#004000" },
        Search = { fg = "#000000", bg = "#00ff00" },
        StatusLine = { fg = "#00ff00", bg = "#0a0a0a" },
        StatusLineNC = { fg = "#008000", bg = "#0a0a0a" },
      },
      dim_inactive = false,
  ...
  elseif theme_name == "zone_green" then
    local zone_green = require("themes.zone_green")
    zone_green.setup()
  else
...
M.show_theme_picker = function()
  local theme_list = {}
  for i, theme in ipairs(M.themes) do
    table.insert(theme_list, i .. ". " .. theme.display)
  end
  
  local choice = vim.fn.input("Select theme (1-" .. #M.themes .. "): ")
  local index = tonumber(choice)
  
  if index and index >= 1 and index <= #M.themes then
    local selected_theme = M.themes[index]
    M.set_theme(selected_theme.name)
  else
